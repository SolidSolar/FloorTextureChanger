...
def hough_transform(img):
    edges = cv2.Canny(img, 25, 30, apertureSize=3)  # Canny edge detection

    lines = cv2.HoughLines(edges, 1, np.pi / 180, 30)  # Hough line detection
    print(lines)
    hough_lines = []
...
def find_vanishing_point(origImage, grid_size, intersections):
    img = np.copy(origImage)
    # Image dimensions
